...
void run_simulation(string pwd, double eta, double kappa){
  ...
  vector<double> radius_nucleus = {11*scale, 7*scale};
  depth = width;
  ...
  string s1 = "cell_0_eta="+to_string(mycell.eta)+"_k="+to_string(mycell.kappa)+".vti";
  string s2 = "nucleus_0_eta="+to_string(mycell.eta)+"_k="+to_string(mycell.kappa)+".vti";
  string s3 = "environment_0_eta="+to_string(mycell.eta)+"_k="+to_string(mycell.kappa)+".vti";
  ...
  if (v0_cell<1e-3) {
    ...
    evolve(mycell, mygrooves.grid, velocity, velocity_nuc,  dt, time, pwd, 2);
...
int main(){
  ...
  vector<double> etas = {0.7};//{0.1, 0.3, 0.5, 0.7, 0.9, 1.1, 1.3, 1.5, 1.7, 1.9};
  vector<double> kappas = {0.1};//{0.1, 0.5, 1, 1.5, 2, 2.5, 3};
